...
class ShieldAPI:
    ...
    def __init__(self, token_path=None, token_history_path=None, proxies=None, headers=None):

        if token_path is None:
            self.token_path = "./.shield_access_token"
        else:
        ...
        self.token_history_path = token_history_path
    ...
    # ===================================================
    # Handling of token authorized for public queries 
    # ===================================================
    ...
    def __loadToken(self):
        ...
        if self.api_token and int(self.api_token["expire_time"]) > int(time.time()):
            pass
    ...
    def __refresh_token(self):
        nfl_token = None
        response = requests.get("https://www.nfl.com/scores/2019/REG1",proxies=self.proxies)
        for line in response.content.split('\n'):
            if "api_token" in line:
                nfl_token = json.loads(line.split("=")[1].strip()[:-1])

        self.api_token = {
            "api_token"     : nfl_token,
            "expire_time"   : int(time.time()+(int(nfl_token["expires_in"])/2))
        }
        with open(self.token_path,'w') as f:
            json.dump(self.api_token,f)

        if self.token_history_path is not None:
            with open(self.token_history_path,'a') as f:
                f.write(str(datetime.datetime.now())+" : "+self.api_token['api_token']['access_token']+"\n")

    def __setAuthHeader(self):
        if self.api_token is None or self.api_token["expire_time"] < time.time():
            self.__refresh_token()
        self.session.headers.update({
            "Authorization" :   "Bearer "+self.api_token["api_token"]["access_token"]
        })
